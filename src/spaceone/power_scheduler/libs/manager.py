...
from spaceone.power_scheduler.libs.schema.region import RegionResource, RegionResponse
...
class AzureManager(BaseManager):
    ...
    def set_connector(self, secret_data):
        if self.connector_name is None:
            self.connector_name = self.locator.get_connector(self.connector_name)
            self.connector_name.get_connect(secret_data)
