...
class TestPowerSchedulerService(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # cls.resource_data =
        cls.vmss_manager = AzureVmScaleSetManager(azure_vm_connector=cls.vmss_connector)
        super().setUpClass()
    ...
    def test_start_manager_vmss(self, *args):
        # Start to - VMs
        secret_data = self.azure_credentials
        resource_type = 'VmScaleSet'

        resource_data = {
            'resource_id': '/subscriptions/3ec64e1e-1ce8-4f2c-82a0-a7f6db0899ca/resourceGroups/JIYOON-RG-0401/providers/Microsoft.Compute/virtualMachineScaleSets/jiyoon-vmss-always-runs',
            'cloud_service_group': 'Compute',
            'cloud_service_type': 'VmScaleSet',
            'subscription_id': '3ec64e1e-1ce8-4f2c-82a0-a7f6db0899ca',
            'resource_group_name': 'jiyoon-rg-0401',
            'vm_name': 'jiyoon-vmss-always-runs'
        }
        ...
        controller_svc = ControllerService({})
        controller_manager = controller_svc.call_manager(resource_type)
        response = controller_manager.start(params)
        pprint(response)
